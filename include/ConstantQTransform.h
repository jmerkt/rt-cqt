...
namespace Cqt
{
    ...
    */
    ...
    {
    public:
        ScheduleElement(const int sample, const int octave, const int delayOctaveRate, const int synthesisOffset = 0) :
            sample_(sample),
            octave_(octave),
    ...
    */
    ...
    public:
        ...
        void initKernels(const CplxVector *const kernelArray,
                         const CplxVector *const kernelArrayInverse,
                         const std::vector<int> *const kernelMask,
                         const std::vector<int> *const kernelMaskInv);
        ...
        static void calculateInverseWindow(double *const windowData,
                                           double *const invWindowData,
                                           const int size,
                                           const int hopSize);
    ...
    template <int B>
    inline void TransformationHandler<B>::initKernels(const CplxVector *const kernelArray,
                                                      const CplxVector *const kernelArrayInverse,
                                                      const std::vector<int> *const kernelMask,
                                                      const std::vector<int> *const kernelMaskInv)
    {
    ...
    inline void TransformationHandler<B>::cqtTransform(const ScheduleElement schedule)
    {
        ...
        mStageInputBuffer->pullDelayBlock(mFftInputBuffer.data(),
                                          static_cast<int>(Fft_Size) + schedule.delayOctaveRate() - 1,
    ...
    inline void TransformationHandler<B>::calculateWindow(double *const windowData, const int size)
    {
        ...
        {
            windowData[i] = (1. / 2.) * (1. - std::cos((2. * audio_utils::Pi<double>() * static_cast<double>(i)) /
                                                       static_cast<double>(size - 1)));
        }
    }

    template <int B>
    inline void TransformationHandler<B>::calculateInverseWindow(double *const windowData,
                                                                 double *const invWindowData,
                                                                 const int size,
                                                                 const int hopSize)
    {
    ...
    */
    ...
    public:
        ...
        inline CqtBufferType *getOctaveCqtBuffer(const int octave)
        {
            return mTransformationHandlers[octave].getCqtBuffer();
        };
        inline BufferPtr getOctaveOutputBuffer(const int octave)
        {
            return mTransformationHandlers[octave].getOutputBuffer();
        };
        ...
        mFftAllocation(Fft_Size)
    {
        ...
        {
            mTransformationHandlers[octave].initBuffers(mFilterbank.getStageInputBuffer(octave),
                                                        mFilterbank.getStageOutputBuffer(octave));
    ...
    inline void ConstantQTransform<B, OctaveNumber>::initFs(double fs, const int blockSize)
    {
        ...
        {
            // latency per octave
            mLatencySamples[octave] = static_cast<size_t>(mHopSizes[octave]) *
                                      static_cast<size_t>(std::pow(2, octave)) *
                                      static_cast<size_t>(std::pow(2, mFilterbank.getOriginDownsampling()));
    ...
    inline void ConstantQTransform<B, OctaveNumber>::calculateKernels()
    {
        ...
        for (int k = 0; k < B; k++)
        {
            ...
            {
                mKernelStorageTime[k][n] = std::conj(
                    (1. / static_cast<double>(Fft_Size)) * window[n] *
                    std::exp(-1i * 2. * audio_utils::Pi<double>() * static_cast<double>(n) * (fk / mSampleRates[0])));
                mKernelStorageTimeInv[k][n] = std::conj((1. / static_cast<double>(Fft_Size)) * window[n] *
                                                        std::exp(-1i * 2. * audio_utils::Pi<double>() *
                                                                 static_cast<double>(n) * (fkInv / mSampleRates[0])));
            }
        }
    ...
    inline void ConstantQTransform<B, OctaveNumber>::inputBlock(double *const data, const int blockSize)
    {
        ...
        {
            for (int octave = (OctaveNumber - 1); octave >= 0;
                 octave--) // starting with lowest pitched octave for historical reasons
            {
                ...
                {
                    mSampleCounters[octave] = 0;
                    const int delayOctaveRate = static_cast<int>(static_cast<double>(processedInputSize - i - 1) *
                                                                 mSampleRatesByOriginRate[octave]);
                    if (firstDelayOctaveRate[octave] < 0)
                    {
                        firstDelayOctaveRate[octave] = delayOctaveRate;
                    }
                    const int synthesisOffset = firstDelayOctaveRate[octave] - delayOctaveRate;
                    mCqtSchedule.push_back({i, octave, delayOctaveRate, synthesisOffset});
                }
            }
        }
    };
